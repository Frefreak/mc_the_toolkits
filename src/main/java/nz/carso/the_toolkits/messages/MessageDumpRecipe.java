...
public class MessageDumpRecipe implements AbstractMessage<MessageDumpRecipe> {
    ...
    public void handle(MessageDumpRecipe msg, Supplier<NetworkEvent.Context> ctx) {
        ...
        entityPlayer.sendSystemMessage(Component.literal(String.format("%d recipes before transform", mrs.size())));
        ...
        entityPlayer.sendSystemMessage(Component.literal(String.format("got %d entries after transformation", result.size())));
        Gson gson = new GsonBuilder().setPrettyPrinting().create();
        String content = gson.toJson(result);
        ctx.get().enqueueWork(() -> {
            if (failed.size() > 0) {
                entityPlayer.sendSystemMessage(Component.literal(
            ...
            if (ok) {
                ...
                entityPlayer.sendSystemMessage(Component.literal("save process failed, see log for more info"));
            }
        });
